...
export const defaultSubscriptionRetryPolicy = {
    maxAttempts: 3,
    baseDelayMs: 1_000,
    maxDelayMs: 60_000,
    jitterRatio: 0.1,
...
export function assertRetryPolicy(policy) {
    if (!Number.isInteger(policy.maxAttempts) ||
        ...
        throw new SubscriptionQueueError("subscription_queue_invalid_retry_policy", "Invalid subscription queue retry policy.");
    }
}
export function computeBackoffDelayMs(input) {
    assertRetryPolicy(input.policy);
    const exponent = Math.max(0, input.attempt - 1);
    const raw = Math.min(input.policy.maxDelayMs, input.policy.baseDelayMs * 2 ** exponent);
